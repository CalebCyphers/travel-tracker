const domUpdate = {
  ...
  displayUpcomingTrips(user, travelRepository) {
    ...
    user.upcomingTrips.forEach(trip => {
      ...
      upcomingTripsContent.innerHTML += `<p class='upcoming-trip'>To: ${destination.destination} <br>• Number of Travelers : ${trip.travelers}<br>• Start Date: ${trip.date}<br>• Quest Duration: ${trip.duration} days.</p>`
    })
  }
}
